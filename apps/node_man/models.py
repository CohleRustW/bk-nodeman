...
class Host(models.Model):
    ...
    def special_proxy_communication_field(self) -> str:
        ...
        """
        proxy_communication_ip_field: str = (
            GlobalSettings.get_config(key=GlobalSettings.KeyEnum.PROXY_COMMUNICATION_IP_FIELD.value) or "inner_ip"
        )
        try:
            communication_ip: str = getattr(self, proxy_communication_ip_field, "") or self.inner_ip
        except AttributeError:
            raise ProxyCommunicationConfigError
        return communication_ip
