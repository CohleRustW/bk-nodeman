...
get_pid_by_comm_path () {
    ...
    local _pids pids
    local pid
    read -r -a _pids <<< "`ps -eo pid,pgid,comm | grep $comm |awk '{print$1}'`"
...
get_config () {
    ...
    local config=(agent.conf bscp.yaml plugin_info.json)
...
bulk_report_step_status () {
    ...
    log_total_line=$(wc -l <"$log_file" |awk '{print $1}')
...
check_disk_space () {
    local dir=$1
    if df -k "$TMP_DIR" | awk 'NR!=1 && $3 >= 100*1024 {x=1;}END{if (x== 1) {exit 0} else {exit 1}}'; then
